...
function demo_ASIFT(file_img1, file_img2, imgOutVert, imgOutHori, matchings, descriptor, keys, flag_resize)
...
elseif (nargin == 7)
    ...
    disp('          matchings.txt descriptor.txt keys.txt [Resize option: 0/1]'); 
    ...
    disp('- descriptor.txt keys.txt: ASIFT keypoints of the two images.')
...
if (ismac == 1)
    ...
    imgOutVert ' ' imgOutHori ' ' matchings ' ' descriptor ' ' keys];
...
%read ASIFT detector matrix
%descript = sprintf('Loading asift_%s_%04d descriptor\n', condition, loop);
%fprintf('### %s\n\n', descript);

%asift_descriptor = load(descriptor);
%asift_descriptor
%fprintf('asift_descriptor size:');
%size(asift_descriptor)

%asift_refDescriptor = load();
...
%fprintf('Matching score: %d/%d * 100 = ', size(scores, 2), size(asift_refDescriptor, 1));
%size(scores, 2)/size(asift_refDescriptor, 1) * 100
